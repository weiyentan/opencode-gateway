...
imports ``redact_dict`` (``app.core.secrets``) and
``_extract_part_columns`` / ``_truncate_json_value`` (``app.api.ingest``)
...
redact → extract → truncate pipeline live ingest applies, with the same
...
from app.api.ingest import _extract_part_columns, _truncate_json_value  # noqa: E402
from app.core.config import get_settings  # noqa: E402
from app.core.secrets import redact_dict  # noqa: E402
...
# convention).  last_seen_at is refreshed to the backfill run time — the
...
def _recompute_tool_call(
    ...
    *,
    ...
    """Recompute the expected ``observed_tool_calls`` columns from part data.

    Applies the exact live-ingest pipeline — :func:`redact_dict` →
    ``_extract_part_columns`` → ``_truncate_json_value`` — using the same
    function objects live ingest calls, so the backfill derivation is
    identical by construction (ADR 0015 equivalence).  Re-applying
    :func:`redact_dict` to stored data is a no-op: ``observed_parts.data``
    was redacted before persistence and the ``"***"`` markers never match
    secret-like key patterns.

    Returns ``(tool_name, tool_status, tool_input, tool_output)`` with
    ``tool_input`` / ``tool_output`` truncated to
    ...
    re-extractable), a non-tool type, or a missing tool name.  Live
    ingest would never have written such a projection row, so the
    backfill cannot either; the caller counts the part skipped.
    """
    redacted = redact_dict(data)
    part_type, tool_name, tool_status, tool_input, tool_output = _extract_part_columns(redacted)
    if part_type != "tool" or tool_name is None:
...
def _tool_call_matches(row: asyncpg.Record, expected: tuple) -> bool:
    ...
    return (
        ...
        and row["tool_input"] == tool_input
        and row["tool_output"] == tool_output
    )
...
async def _upsert_tool_call(
    ...
    *,
    ...
    ``now`` — the projection was (re)confirmed at backfill time.
    """
    tool_name, tool_status, tool_input, tool_output = expected
    await conn.execute(
        UPSERT_TOOL_CALL_SQL,
