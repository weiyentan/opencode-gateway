#!/usr/bin/env python3
"""Daily refresh of recent ``usage_dashboard_daily`` buckets (issue #737).

Kubernetes CronJob entry point for the usage dashboard daily refresh.  It
recomputes the pre-aggregated usage dashboard rollup (``usage_dashboard_daily``,
migration 0048) for a small, configurable window of recent UTC days — by
default **today and yesterday** — by delegating the per-bucket metric
computation to the shared recomputation engine in
``app.core.usage_dashboard_daily`` (issue #735): it discovers every
``(day, provider)`` bucket that carries activity in the window, then calls
:func:`~app.core.usage_dashboard_daily.recompute_bucket` for each.

The engine derives every additive metric from the canonical ``usage_events``
table, so the rollup stays a rebuildable projection of source facts.  Unlike
the one-shot backfill, this refresh is bounded to the configured day window and
...
(:func:`~app.core.usage_dashboard_daily.acquire_bucket_lock`, a
transaction-scoped ``pg_advisory_xact_lock`` in the
:data:`~app.core.usage_dashboard_daily.USAGE_DASHBOARD_LOCK_CLASS` namespace)
before calling :func:`~app.core.usage_dashboard_daily.recompute_bucket`, so
...
Usage:
    python scripts/refresh_usage_dashboard_daily.py [--days-back N] [--as-of YYYY-MM-DD]
...
logger = logging.getLogger("refresh_usage_dashboard_daily")
...
# SQL
#
# Bucket discovery.  The ``usage_events`` table buckets by
# ``reported_at`` into UTC calendar days; the query collects every
# ``(day, provider)`` pair that has activity in the configured window
# ($1 start, $2 end).  ``DISTINCT`` collapses the rows to the distinct
# pairs the engine recomputes.  The actual metric computation is the
# engine's responsibility — this query only decides *which* buckets are
# worth recomputing.
# ---------------------------------------------------------------------------

DISCOVERY_SQL = """
    SELECT DISTINCT ON (day, provider)
           day, provider
    FROM (
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Recompute recent usage_dashboard_daily buckets from the "
        "canonical usage_events table (default: today and yesterday, UTC).",
    )
    parser.add_argument(
        "--days-back",
        dest="days",
...
async def _run_windowed_recompute(
    ...
    """Recompute every active ``usage_dashboard_daily`` bucket in the window.

    Discovers the distinct ``(day, provider)`` pairs with activity in
    ``[start_day, end_day]`` and recomputes each one through the shared
    engine.  Every bucket is recomputed in its own transaction, under
...
async def main(argv: list[str] | None = None) -> int:
    ...
    logger.info(
        "Usage dashboard daily refresh window: %s .. %s (%d day(s)).",
        start_day,
        end_day,
        args.days,
    )
    ...
    try:
        async with pool.acquire() as conn:
            if not await _try_acquire_refresh_lock(conn):
                logger.warning(
                    "Another usage dashboard daily refresh holds the aggregate "
            ...
            try:
                ...
                logger.info(
                    "Recomputed %d usage_dashboard_daily bucket(s) for window "
                    "%s .. %s.",
                    recomputed,
                    start_day,
                    end_day,
