...
| `GET` | `/api/v1/afk-outcomes/runs/{afk_run_id}` | Full chain for one run: run aggregate, EngineeringOutcome, engineering entities grouped by type (issues, change_requests, reviews, commits, merge_events) each carrying correlation and lineage provenance (`correlation_method`, `correlation_confidence`, `evidence`, `resolver_version`, `correlation_source`, `owning_change_request_id`) and a `provisional` marker, linked sessions with usage/cost aggregates, `parent_session_id` for the nested session tree (issue #575), distinct agents, and a run-level usage aggregate (Active Tokens = input + output; cache read/write as siblings). 404 for unknown `afk_run_id`. |
...
| **Agent Runs** | `/api/v1/usage/agent-runs` | Agent run table with session title, status, model, provider, project/worktree, todo/files, costs, compact tokens, cache read/write, reasoning, last updated, and children. Detail overlay includes a Token Breakdown section (input/output/cache read/cache write/reasoning, cache hit ratio, provider) alongside session context (title, model, code changes) and todo progress (issue #557). The AFK Outcomes provenance session links now carry `parent_session_id` (issue #575) so the nested root→child tree is explicit |
| **AFK Outcomes** | `/api/v1/afk-outcomes/runs`, `/api/v1/afk-outcomes/runs/{afk_run_id}` | Repository-first workflow: **Repository Summary** (normalized GitHub/GitLab list with provider badges, AFK activity counts, date-range scoping, loading/empty/stale-on-error states) → **Change Request List** for the selected repository (all PRs/MRs in the period with provider-specific PR/MR labels under the canonical `change_request` vocabulary, AFK-linked highlighting, AFK-only filter with no browser-side joins, selection preserved across date-range changes) → **Change Request Provenance Timeline** (chronological develop/review executions with phase, timestamps, status/outcome, duration, AWX job IDs, OpenCode session IDs, usage/cost, merge state vs issue-closure state, RunStatus distinct from EngineeringOutcomeStatus) with **nested session relationships** (root vs child via `parent_session_id`, deep-link to the Agent Run detail overlay, explicit missing/unresolved markers) and **relationship certainty** (resolved vs provisional/inferred distinction, confidence/method/evidence/resolver_version, dedicated Unresolved Relationships view for `ambiguous`/`unmatched`/`parked` with provenance). All views use deterministic `frontend/fixtures/*` GitHub/GitLab parity fixtures for offline regression (`node frontend/tests/test_pure_functions.js`), plus an opt-in live-provider E2E harness (`scripts/afk_e2e_test.py`, `docs/afk-e2e-validation.md`). Sessions in the chain/provenance detail that carry a resolvable internal session ID are clickable and open the existing Agent Run detail overlay (the AFK overlay closes first); unresolved sessions stay non-clickable |
...
│   ├── afk_e2e_test.py           # Opt-in GitHub/GitLab lifecycle E2E harness — disposable repos, bounded polling, redacted evidence (issue #578; see docs/afk-e2e-validation.md)
...
│   ├── fixtures/                 # Deterministic GitHub/GitLab AFK fixtures — complete/incomplete lifecycles, repeated cycles, root/child sessions, provisional/unresolved states (issue #571)
│   └── tests/                    # VM-sandbox regression coverage for the repository-first flow (issue #577; node frontend/tests/test_pure_functions.js)
├── tests/                        # Foundation tests (more to be added)
├── docs/
│   ├── adr/                      # Architecture Decision Records
│   ├── afk-outcome-validation.md # AFK reconstruction validation findings (#450)
│   └── afk-e2e-validation.md     # Opt-in GitHub/GitLab lifecycle E2E operator guide — explicit command, env vars, prerequisites, scenario, evidence, bounded polling, cleanup (issue #578)
