...
# ── API-only gateway — root path ──────────────────────────────────────


@pytest.mark.asyncio
async def test_root_path_returns_404(monkeypatch):
    """The root path returns 404 — the Gateway no longer serves Aurora Glass."""
    monkeypatch.setenv("GATEWAY_API_KEY", "test-api-key")
    from httpx import ASGITransport, AsyncClient
    ...
    async with AsyncClient(
        ...
        # Root path returns 404 — no frontend route
