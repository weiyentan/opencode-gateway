"""Workspace API endpoints — pin and cleanup lifecycle management."""
...
# ---------------------------------------------------------------------------
#
# _PORT_LOCK_KEY:    Used by ``allocate_port()`` to serialise port allocation
#                    across concurrent workspace-creation requests so that two
#                    callers never receive the same port.
# _CLEANUP_LOCK_CLASS:  High 32 bits of the two-argument advisory lock used
#                    to guard cleanup of a specific workspace.  The low 32
#                    bits are derived from ``workspace.id.int & 0xFFFFFFFF``.
...
def _build_response(row: asyncpg.Record) -> WorkspacePydantic:
    """Convert an asyncpg workspace row to a WorkspacePydantic response."""
