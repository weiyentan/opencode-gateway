...
async def _record_canonical_event(
    ...
    from app.core.reconciliation import acquire_canonical_event_lock
    ...
    async with conn.transaction():
        ...
        if existing is not None:
            # Another request already created the event while we were
            # waiting for the lock (re-read-after-commit).
