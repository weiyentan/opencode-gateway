...
class Runner(Base):
    ...
    # admin_status: operator-set (online, offline, maintenance), defaults to 'online'
    admin_status: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True, default="online"
    )
    # health_status: observation-derived (HEALTHY, BLOCKED_DISK_PRESSURE, BLOCKED_MEMORY_PRESSURE, UNKNOWN), defaults to 'UNKNOWN'
    health_status: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True, default="UNKNOWN"
    )
