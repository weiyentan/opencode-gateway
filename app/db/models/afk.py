...
class ExecutionBinding(Base):
    ...
    ``awx_job_id`` is the unique idempotency key — one row per AWX job.
    Provider resource identity is NOT unique — allowing multiple failed and
    successful executions for the same change request.  ``failure_reason``
    and ``failure_summary`` carry bounded diagnostic metadata; raw
    ``extra_vars``, stdout, prompts, tokens, or arbitrary AWX payloads are
    never stored.
    """
    ...
    awx_job_id: Mapped[int] = mapped_column(
        "awx_job_id", BigInteger(), nullable=False
    )
