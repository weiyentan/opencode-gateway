...
- **Redaction boundary invariant (issue #564).** `failure_summary` redaction is applied once at the API schema layer before persistence — the domain model stores the already-redacted value as-is and never re-redacts. Direct repository callers are responsible for their own redaction. Truncation is a Python character slice (`[:1000]`), applied after redaction, never a byte slice.
- **Completed-outcome invariant (issue #564).** A `completed` execution carries no failure metadata: non-null `failure_reason` or `failure_summary` alongside `outcome == completed` is rejected with `422` on both POST and PATCH; the repository's after-merge check additionally rejects a completed transition whose merged state still carries a stored `failure_summary`.
