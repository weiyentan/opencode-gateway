...
A future implementation slice would include Alembic migration(s), persistence/model code, `POST /api/v1/afk/executions`, and read paths by AWX job or provider resource. Implementation is deferred — this ADR documents the design only.
