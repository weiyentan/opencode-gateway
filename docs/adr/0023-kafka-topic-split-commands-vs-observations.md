...
7. **Migration**: create topics → deploy consumer on new topic → validate consumer health (lag trending to zero, no errors) → switch producer → verify. The reconcile loop (1h cadence, 24h window) backfills the small cutover gap; no dual-write/dual-consume compatibility code. Recoverable consumer failures leave the producer on `engineering.events.normalized` while Kafka retains the backlog; repair and replay remain idempotent via `delivery_log UNIQUE(provider, delivery_id)`. Roll back the producer only if webhook handling or the `afk.events` command/AWX path regresses.
...
| 2 | Non-draft PR/MR opened by an authorized issuer without `afk` label | Command (`pr_mr_opened`) AND observation |
...
| 11 | Recoverable consumer failure | Producer remains on normalized topic; repaired consumer drains retained backlog idempotently |
