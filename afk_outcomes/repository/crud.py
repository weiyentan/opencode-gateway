"""Core CRUD seam of the ``AsyncpgOutcomeRepository`` (issue #683).

Split out of the former monolithic ``afk_outcomes/repository.py``: run-aggregate
persistence (:meth:`~afk_outcomes.repository.AsyncpgOutcomeRepository.save` /
``get``), exact resource<->session associations (``save_associations``), and
live event recording (``record_event``), plus the shared serialization helpers
and identity constants they rely on.  The methods are composed onto the facade
...
class _CrudRepositoryMixin:
    'Core CRUD methods of :class:`AsyncpgOutcomeRepository`.'
